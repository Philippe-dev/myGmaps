...
if (!$core->error->flag())
{
	...
	form::hidden(array('add_map_filters'),'myGmaps').
	...
	if (isset($maps_array)) {
		for ($i = 0; $i < sizeof($maps_array); ++$i) {
			$maplist .= '<p class="maplist" style="display:none">'.form::checkbox(array('entries[]'),$maps_array[$i],'true','','','').'</p>';
		}
	}
